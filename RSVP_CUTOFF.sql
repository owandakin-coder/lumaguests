-- ============================================================
-- RSVP CUTOFF — Luma Guests
-- Run in Supabase SQL Editor
-- ============================================================

-- 1. Add public_rsvp_enabled column (default open)
alter table public.events
  add column if not exists public_rsvp_enabled boolean default true;

-- Backfill: NULL → true (open) for all existing events
update public.events
set public_rsvp_enabled = true
where public_rsvp_enabled is null;

-- 2. Rewrite public_rsvp_register — adds rsvp_closed gate
...
-- Done.
-- New behaviour:
--   • public_rsvp_enabled = false  →  rsvp_closed error
--   • Toggle in Event Studio saves this field
--   • Personal RSVP links (respond_to_rsvp) are NOT affected —
--     existing invited guests can still respond via their link
-- ============================================================
